...
import yalong.site.json.entity.match.Teams;
...
public class MatchHistoryUtil {
    ...
    public static ImageIcon getItemImageIcon(Integer id, Integer itemIconWidth, Integer itemIconHeight) {
        if (id != 0) {
            try {
                String iconPath = AppCache.itemList.stream().filter(item -> item.getId().equals(id)).findFirst().get().getIconPath();
    ...
    public static ImageIcon getPerkImageIcon(Integer id, Integer perkIconWidth, Integer perkIconHeight) {
        if (id != 0) {
            try {
                String iconPath = AppCache.perkList.stream().filter(perk -> perk.getId().equals(id)).findFirst().get().getIconPath();
    ...
    public static ImageIcon getPerkStyleImageIcon(Integer id, Integer perkStyleIconWidth, Integer perkStyleIconHeight) {
        if (id != 0) {
            try {
                String iconPath = AppCache.perkStyleList.stream().filter(perk -> perk.getId().equals(id)).findFirst().get().getIconPath();
    ...
    public static String getPerkData(Stats stats) {
        ...
        if (stats.getPerk0() != 0) {
            PerkBO perk0 = AppCache.perkList.stream().filter(perk -> perk.getId().equals(stats.getPerk0())).findFirst().get();
        ...
        if (stats.getPerk1() != 0) {
            PerkBO perk1 = AppCache.perkList.stream().filter(perk -> perk.getId().equals(stats.getPerk1())).findFirst().get();
        ...
        if (stats.getPerk2() != 0) {
            PerkBO perk2 = AppCache.perkList.stream().filter(perk -> perk.getId().equals(stats.getPerk2())).findFirst().get();
