 ...
 */
@Slf4j
public class BlackListHistoryDetail extends JPanel {
    ...
    public BlackListHistoryDetail(SpgGames bo, List<BlackListPlayer> blackListPlayers) throws IOException {
        ...
        for (int i = 0; i < teamOne.size(); i++) {
            if (teamOne.get(i) != null && bo.getParticipants().get(i) != null) {
                this.add(buildTeamData(teamOne.get(i), scoreLevelMap.get(GameConstant.TEAM_ONE).get(i), bo.getGameId(), blackListPlayers));
            }
        }
        for (int i = 0; i < teamTwo.size(); i++) {
            if (teamTwo.get(i) != null && bo.getParticipants().get(i) != null) {
                this.add(buildTeamData(teamTwo.get(i), scoreLevelMap.get(GameConstant.TEAM_TWO).get(i), bo.getGameId(), blackListPlayers));
            }
        }
    }

    public static BlackListHistoryDetail build(BlackListBO bo) throws IOException {
        return new BlackListHistoryDetail(bo.getSpgGames(), bo.getBlackListPlayers());
    }
     ...
     * @return JPanel
     */
    private JPanel buildTeamData(SpgParticipants data, ScoreLevelBO scoreLevel, Long gameId, List<BlackListPlayer> blackListPlayers) throws IOException {
        JPanel jPanel = new JPanel();
        GridBagLayout layout = new GridBagLayout();
        jPanel.setLayout(layout);
        jPanel.setBackground(ColorConstant.DARK_THREE);
        Optional<BlackListPlayer> first = blackListPlayers.stream().filter(item -> item.getPuuid().equals(data.getPuuid())).findFirst();
        String remark = null;
        if (first.isEmpty()) {
            jPanel.setBackground(MatchHistoryUtil.getBackGroundColor(data.isWin()));
        }
        jPanel.setBorder(BorderFactory.createLineBorder(Color.GRAY, 1, true));
        ...
        name.addMouseListener(new BlackListNameLabelListener(player, currentSpgGame, first.isPresent(), name));
        ...
        order.setPreferredSize(new Dimension(FrameSetting.MATCH_WIDTH / 20, FrameSetting.MATCH_HEIGHT / 50));
        ...
        visionScore.setPreferredSize(new Dimension(FrameSetting.MATCH_WIDTH / 15, FrameSetting.MATCH_HEIGHT / 50));
...
@Slf4j
class BlackListNameLabelListener extends MouseAdapter {
    private SpgGames spgGames;
    private boolean isPlackListPlayer;
    private JLabel label;
    private Player player;

    public BlackListNameLabelListener(Player player, SpgGames spgGames, boolean isPlackListPlayer, JLabel label) {
        this.player = player;
    ...
    private void addBlackList(BlackListPlayer player) {
        ...
        if (FileUtil.exist(file)) {
            ...
            Optional<SpgParticipants> me = spgGames.getParticipants().stream().filter(item -> item.getPuuid().equals(mePuuid)).findFirst();
            if (me.isPresent()) {
                players.add(player);
                boolean win = me.get().isWin();
    ...
    private void closeDetail() {
        ...
        if (blackLists.isEmpty()) {
            ...
            FrameInnerCache.blackListMatchPanel.repaint();
            FrameInnerCache.blackListMatchPanel.revalidate();
        }
