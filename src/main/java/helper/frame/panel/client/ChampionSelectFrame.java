...
public class ChampionSelectFrame extends JFrame {
    ...
    public ChampionSelectFrame(String title) {
    ...
    private void initNorthUI() {
        ...
        helpButton.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                if (e.getButton() == MouseEvent.BUTTON1) {
                    ...
                    sb.append("<html><font color='red'>无视队友预选</font>开关为排位自己的首选英雄被ban 顺位英雄为队友预选英雄是否选择\n<html>");
                    sb.append("<html><font color='red'>选择英雄</font>从点击右边英雄头像鼠标左键\n<html>");
                    sb.append("<html><font color='red'>英雄置顶</font>从点击左边英雄头像鼠标左键可以置顶\n<html>");
                    sb.append("<html><font color='red'>英雄取消</font>从点击左边英雄头像鼠标右键可以取消秒选\n<html>");
                    sb.append("<html><font color='red'>匹配模式</font>匹配秒选只选择第一个置顶英雄<font color='red'>如果选慢了LOL客户端会给你随机一个英雄 没有第二次机会</font> \n<html>");
                    sb.append("<html><font color='red'>骰子模式</font>大乱斗秒选优先选择置顶 如果手动交换英雄 这个英雄不会再抢 从第二个以此类推\n<html>");
                    sb.append("<html><font color='red'>排位模式</font>排位根据每个位置的配置预选置顶的英雄 经过队友预选和双方ban位选择最后剩下的英雄 并且默认选择配置中还有的英雄但不锁定<html>");
        ...
        // 创建单选按钮
        BaseCheckBox championSelectFlag = new BaseCheckBox();
        championSelectFlag.setText("排位秒选预选");
        championSelectFlag.addItemListener(e -> {
            AppCache.settingPersistence.setChampionSelectFlag(e.getStateChange() == ItemEvent.SELECTED);
            FrameConfigUtil.save();
        });
        BaseCheckBox teamSelectFlag = new BaseCheckBox();
        teamSelectFlag.setText("无视队友预选");
        teamSelectFlag.setSelected(AppCache.settingPersistence.getPickTeammateFlag());
        teamSelectFlag.addItemListener(e -> {
            AppCache.settingPersistence.setPickTeammateFlag(e.getStateChange() == ItemEvent.SELECTED);
            FrameConfigUtil.save();
        });
        ...
        contentPanel.add(championSelectFlag, "w 100!, h 40!, gapleft 5");
    ...
    private void initCenterUI() {
        ...
        leftMainPanel.setBorder(BorderFactory.createTitledBorder("已选择的英雄"));
        ...
        rightPanel.setBorder(BorderFactory.createTitledBorder("可选英雄"));
    ...
    private void updateRightPanel(BaseComboBox<ItemBO> positionSelect, BaseComboBox<ItemBO> roleSelect, JTextField searchTextField) {
        ...
        List<TencentChampion> champions = GameDataCache.allChampionName;
    ...
    private JPanel createChampionPanel(TencentChampion champion, boolean isSelectedPanel) {
        ...
        if (isSelectedPanel) {
            ...
            overlayIcon = new ImageIcon(Objects.requireNonNull(this.getClass().getResource("/assets/pick.png"))); // 默认覆盖图标路径
        }
    ...
    protected BaseComboBox<ItemBO> createCategorySelect() {
        ...
        categorySelect.addItem(new ItemBO("bench", "随机模式"));
    ...
    protected void addChampion(String category, Integer championId) {
        ArrayList<Integer> idList = AppCache.settingPersistence.getPickMap().get(category);
    ...
    protected void initLeftJPane() {
        ...
        for (Map.Entry<String, ArrayList<Integer>> entry : AppCache.settingPersistence.getPickMap().entrySet()) {
